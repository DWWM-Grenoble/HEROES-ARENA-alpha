...
export function validateStats(stats) {
    ...
    const valid = total === gameConfig.statPoints && stats.force >= gameConfig.minStatValue  && stats.force <= gameConfig.maxStatValue && stats.agility >= gameConfig.minStatValue && stats.agility <= gameConfig.maxStatValue && stats.magic >= gameConfig.minStatValue && stats.magic <= gameConfig.maxStatValue && stats.defense >= gameConfig.minStatValue && stats.defense <= gameConfig.maxStatValue;
...
export function validateName(nom, existingNames = []) {
    ...
    if (formatted.lenght > gameConfig.maxNameLenght) {
        return { valid: false, error: `Le nom ne peut pas dépasser ${gameConfig.gameConfig.maxNameLenght} caractères`};
    }
